...
export function TeamCard({ team, playersCount }: TeamCardProps) {
  return (
    <Link href={`/team/${team.id}`} className="group block">
      <GlassPanel className="relative h-full overflow-hidden p-5 transition duration-300 hover:-translate-y-1 hover:border-cyan-300/30">
        ...
        <div className="relative flex h-full flex-col justify-between gap-6">
          ...
          <div>
            <p className="font-accent text-xs uppercase tracking-[0.26em] text-cyan-100/70">{team.strapline}</p>
            <div className="mt-4 flex items-center gap-3 text-sm text-white/75">
              <MapPin className="h-4 w-4 text-cyan-200" />
              {team.city}
            </div>
            <div className="mt-4 flex items-center justify-between text-sm">
              <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-white/75">
                {playersCount} player profiles
              </span>
              <span className="font-accent uppercase tracking-[0.2em] text-cyan-100">Enter hub</span>
            </div>
          </div>
        </div>
      </GlassPanel>
